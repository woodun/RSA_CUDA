...
__global__ void MontSQMLadder(cuda_mpz_t * mes1, cuda_mpz_t r2, cuda_mpz_t vn, cuda_mpz_t vn_, int* eBits, int eLength, long long int* divTable) {
	...
	__shared__ cuda_mpz_t tmp[22]; //capacity will become a problem for shared memory with large keys
	__shared__ cuda_mpz_t tmp2[22];
	__shared__ cuda_mpz_t t[22];
	__shared__ cuda_mpz_t _x1[22];
	__shared__ cuda_mpz_t _x2[22];
